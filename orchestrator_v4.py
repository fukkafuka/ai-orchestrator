...
LOCAL_MODEL_PATH = os.path.expanduser("~/ai-orchestrator/llama.cpp/models/llm-jp-3.1-1.8b-instruct4-lora-Q4_K_M-347.gguf")
...
判断基準:
- try/exceptで捕捉され、処理は継続・クラッシュせず動作している例外(警告的なもの、リトライ後に成功しているもの、頻発していても機能面で実害がないもの)は、実害のないノイズとして低優先度に扱い、これらを「重大な問題」として提案しないこと。
- 優先すべきは、サービス停止・機能停止・クラッシュ・繰り返し失敗して回復していない・データ不整合など、実際にユーザーへの実害が出ている問題。
- ログ中に複数のエラーが見られる場合、単に目立つ・出現回数が多いというだけで飛びつかず、実害の大きさで判断すること。
